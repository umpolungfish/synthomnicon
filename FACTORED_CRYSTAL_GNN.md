...
| $T$ | $T_\odot$ | $T_\odot$ | $T_\odot$ | $T_\odot$ | $T_\text{box}$ |
...
All navigators share $P_{\pm}^\text{sym}$, $G_\aleph$, $\Phi_c$ — the core $O_\infty$ signature. Differentiation occurs along $K$ (kinetic regime), $\Omega$ (topological protection), $R$ (relational mode), and $D/T$ (dimensionality/topology). The Ising navigator is the structural outlier: $D_\triangle$, $T_\text{box}$, $K_\text{fast}$, $\Gamma_\text{and}$, $H_0$, $F_\ell$ — reflecting its discrete-lattice, single-pass, local-coupling character.
...
**Structural type:** $\langle D_\triangle;\ T_\text{box};\ R_\text{cat};\ P_{\pm}^\text{sym};\ F_\ell;\ K_\text{fast};\ G_\aleph;\ \Gamma_\text{and};\ \Phi_c;\ H_0;\ n{:}n;\ \Omega_{Z_2} \rangle$
