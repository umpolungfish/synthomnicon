...
class ThurstonNetWithTSpecialist(nn.Module):
    ...
    def forward(
        ...
        if abs(p_h3 - p_h2xr) < self.confusion_margin:
...
def evaluate_t_specialist_on_confusions(
    ...
    with torch.no_grad():
        for _ in range(n_samples):
            ...
            if abs(p_h3 - p_h2xr) < confusion_margin:
                n_confused += 1
...
def feature_importance_ablation(
    ...
    Grammar prediction: Feature 0 (frac_norm_near_one) should have the largest drop
    because it directly indexes the $T_\\text{in}$ product structure ($T_\\odot$ has none).
    """
    feature_names = [
    ...
    with torch.no_grad():
        ...
        print(f"  Grammar prediction: Feature 0 (frac_norm_near_one) → largest drop")
...
def main():
    ...
    print(f"  Training ThurstonNet backbone (fast, 50 epochs for evaluation only)...")
    from train_navigators import train_thurston
    backbone = train_thurston(epochs=50)

    results = evaluate_t_specialist_on_confusions(
        backbone, specialist, n_samples=200, confusion_margin=0.15,
    )

    print(f"\n{'='*68}")
    print(f"SUMMARY")
    print(f"{'='*68}")
    print(f"  T-specialist accuracy on H3/H2×R confusions: {results['acc_specialist']:.1%}")
    ...
    print(f"\n  Next: integrate TTopologySpecialist into ThurstonNetWithTSpecialist")
    print(f"  and compare combined accuracy to the H3 K_trap specialist baseline (94%).")
